/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/J2EE/EJB30/StatelessEjbClass.java to edit this template
 */
package ejb;

import Entity.Booking;
import Entity.CategoryMaster;
import Entity.CityMaster;
import Entity.Events;
import Entity.Facilities;
import Entity.Feedback;
import Entity.GroupMaster;
import Entity.GroupMasterPK;
import Entity.Hostel;
import Entity.Notice;
import Entity.PaymentDetail;
import Entity.PaymentMaster;
import Entity.Room;
import Entity.Rules;
import Entity.StateMaster;
import Entity.Timelist;
import Entity.UserMaster;
import java.sql.Time;
import java.util.Collection;
import java.util.Date;
import javax.ejb.Stateless;
import javax.persistence.EntityManager;
import javax.persistence.PersistenceContext;
import org.glassfish.soteria.identitystores.hash.Pbkdf2PasswordHashImpl;

/**
 *
 * @author meetk
 */
@Stateless
public class PublishBean implements PublishBeanLocal {
    
    @PersistenceContext(unitName = "OHS_PublishAppPU")
    EntityManager em;
    private Pbkdf2PasswordHashImpl pb;

    @Override
    public void addBooking(Integer user_id,Integer room_id,Integer payment_id, String booking_date) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        
        
        UserMaster um = (UserMaster) em.createNamedQuery("UserMaster.findByUserId").setParameter("userId",user_id).getSingleResult();
        Collection<Booking> booking = um.getBookingCollection();
        Room rooms = em.find(Room.class,room_id);
        Collection<Booking> bookings = rooms.getBookingCollection();
        PaymentMaster pm = em.find(PaymentMaster.class,payment_id);
        Collection<Booking> bookingss = pm.getBookingCollection();
        Booking b = new Booking();
        b.setUserId(um);
        b.setRoomId(rooms);
        b.setPaymentId(pm);
        b.setBookingDate(booking_date);
        booking.add(b);
        um.setBookingCollection(booking);
        rooms.setBookingCollection(bookings);
        pm.setBookingCollection(bookingss);
        em.persist(b);
        em.merge(um);
        em.merge(rooms);
        em.merge(pm);

        
    }

    

    @Override
    public void updateBooking(Integer booking_id, Integer user_id, Integer room_id,Integer payment_id, String booking_date) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        UserMaster um = (UserMaster) em.createNamedQuery("UserMaster.findByUserId").setParameter("userId",user_id).getSingleResult();
        Room room = em.find(Room.class, room_id);
        PaymentMaster pm = em.find(PaymentMaster.class,payment_id);
        Booking b = em.find(Booking.class,booking_id);
        b.setUserId(um);
        b.setRoomId(room);
        b.setPaymentId(pm);
        b.setBookingDate(booking_date);
        em.merge(b);
    
    }

    @Override
    public void removeBooking(Integer booking_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       Booking booking = em.find(Booking.class,booking_id);
       em.remove(booking);
    
    }

    
    @Override
    public Collection<Booking> getAllBookings() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Booking.findAll").getResultList();
    
    }

    @Override
    public Collection<Booking> getBookingsOfUserMaster(Integer user_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        UserMaster um = (UserMaster) em.createNamedQuery("UserMaster.findByUserId").setParameter("userId",user_id).getSingleResult();
        return um.getBookingCollection();
    
    }

    @Override
    public Collection<Booking> getBookingsOfRoom(Integer room_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Room room = em.find(Room.class, room_id);
        return room.getBookingCollection();
    
    }
    
    @Override
    public Collection<Booking> getBookingsOfPaymentId(Integer payment_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        PaymentMaster pm = em.find(PaymentMaster.class, payment_id);
        return pm.getBookingCollection();
    
    }

    @Override
    public Collection<Booking> getBookingsByBookingDate(String booking_date) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Booking> bookings = em.createNamedQuery("Booking.findByBookingdate").setParameter("booking_date",booking_date).getResultList();
        return bookings;
    
    }

    @Override
    public Booking findBookingById(Integer booking_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(Booking.class,booking_id);
    }

    @Override
    public void addCategoryMaster(String category_type) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CategoryMaster c = new CategoryMaster();
        c.setCategoryType(category_type);
        em.persist(c);
    }

    @Override
    public void updateCategoryMaster(Integer category_id, String category_type) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        CategoryMaster c = em.find(CategoryMaster.class,category_id);
        c.setCategoryType(category_type);
        em.merge(c);
    }

    @Override
    public void removeCategoryMaster(Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CategoryMaster c = em.find(CategoryMaster.class,category_id);
        em.remove(c);
    }

    @Override
    public String CategoryTypeById(Integer category_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       CategoryMaster category =  em.find(CategoryMaster.class,category_id);
       return category.getCategoryType();
    
    }
    
    @Override
    public Collection<CategoryMaster> getAllCategoryMasters() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("CategoryMaster.findAll").getResultList();
    }

    @Override
    public Collection<CategoryMaster> getCategoryMastersByCategoryType(String category_type) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<CategoryMaster> category = em.createNamedQuery("CategoryMaster.findByCategoryType").setParameter("CategoryType",category_type).getResultList();
        return category;
    }

    @Override
    public CategoryMaster findCategoryMasterById(Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(CategoryMaster.class,category_id);
    }

    @Override
    public void addCityMaster(String city_name, Integer state_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
      StateMaster state = em.find(StateMaster.class, state_id);
      Collection<CityMaster> citys = state.getCityMasterCollection();
      CityMaster city = new CityMaster();
      city.setCityName(city_name);
      city.setStateId(state);
      citys.add(city);
      state.setCityMasterCollection(citys);
      em.persist(city);
      em.merge(state);
    
    }

    @Override
    public void updateCityMaster(Integer city_id, String city_name, Integer state_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CityMaster c = em.find(CityMaster.class,city_id);
        c.setCityName(city_name);
        em.merge(c);
    }

    @Override
    public void removeCityMaster(Integer city_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
       CityMaster city = em.find(CityMaster.class,city_id);
       em.remove(city);
       
    }

    @Override
    public String CityNameById(Integer city_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        CityMaster city = em.find(CityMaster.class,city_id);
        return city.getCityName();
    
    }
    
    

    @Override
    public Collection<CityMaster> getAllCityMasters() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        return em.createNamedQuery("CityMaster.findAll").getResultList();
    }

    @Override
    public Collection<CityMaster> getCityMasterOfStateId(Integer state_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        StateMaster state = em.find(StateMaster.class, state_id);
        return state.getCityMasterCollection();
    }

    @Override
    public Collection<CityMaster> getCityMastersByCityName(String city_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<CityMaster> citys = em.createNamedQuery("CityMaster.findByCityName").setParameter("CityName",city_name).getResultList();
        return citys;
    }

    @Override
    public CityMaster findCityMasterById(Integer city_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        return em.find(CityMaster.class,city_id);
    }

    @Override
    public void addHostel(String hostel_name, Integer city_id,Integer state_id, String phone_no,Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
 
      CityMaster city = em.find(CityMaster.class, city_id);
      StateMaster state = em.find(StateMaster.class, state_id);
      CategoryMaster category = em.find(CategoryMaster.class, category_id);
      
      Hostel hostel = new Hostel();
      
      hostel.setHostelName(hostel_name);
      hostel.setCityId(city);
      hostel.setStateId(state);
      hostel.setPhoneNo(phone_no);
      hostel.setCategoryId(category);
      
      em.persist(hostel);
      
    
    }

   

   

    @Override
    public void updateHostel(Integer hostel_id, String hostel_name, Integer city_id, Integer state_id, String phone_no, Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CityMaster city = em.find(CityMaster.class, city_id);
        StateMaster state = em.find(StateMaster.class, state_id);
        CategoryMaster category = em.find(CategoryMaster.class, category_id);
        Hostel h = em.find(Hostel.class,hostel_id);
        
        h.setHostelName(hostel_name);
        h.setCityId(city);
        h.setStateId(state);
        h.setPhoneNo(phone_no);
        h.setCategoryId(category);
        em.merge(h);
    
    }

    @Override
    public void removeHostel(Integer hostel_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
       Hostel hostel = em.find(Hostel.class,hostel_id);
       em.remove(hostel);
    
    }

    
    @Override
    public Collection<Hostel> getAllHostels() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Hostel.findAll").getResultList();
    
    
    }

    @Override
    public Collection<Hostel> getHostelsOfCityId(Integer city_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CityMaster city = em.find(CityMaster.class, city_id);
        return city.getHostelCollection();
    
    
    
    }

    @Override
    public Collection<Hostel> getHostelsOfStateId(Integer state_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        
        StateMaster state = em.find(StateMaster.class, state_id);
        return state.getHostelCollection();
    
    }

    @Override
    public Collection<Hostel> getHostelsOfCategoryId(Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CategoryMaster category = em.find(CategoryMaster.class, category_id); 
        return category.getHostelCollection();
    
    
    }

    @Override
    public Collection<Hostel> getHostelsByHostelName(String hostel_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        Collection<Hostel> hostels = em.createNamedQuery("Hostel.findByHostelName").setParameter("HostelName",hostel_name).getResultList();
        return hostels;
    
    
    }
    
    @Override
    public Collection<Hostel> getHostelsByPhoneNo(String phone_no) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Hostel> hostels = em.createNamedQuery("Hostel.findByPhoneNo").setParameter("PhoneNo",phone_no).getResultList();
        return hostels;
    
    }
    

    @Override
    public Hostel findHostelById(Integer hostel_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(Hostel.class,hostel_id);
    
    }

    @Override
    public void addPaymentMaster(String payment_mode) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        PaymentMaster p = new PaymentMaster();
        p.setPaymentMode(payment_mode);
        em.persist(p);   
    
    
    }

    @Override
    public void updatePaymentMaster(Integer payment_id, String payment_mode) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        PaymentMaster p = em.find(PaymentMaster.class,payment_id);
        p.setPaymentMode(payment_mode);
        em.persist(p);
    }

    @Override
    public void removePaymentMaster(Integer payment_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        
        PaymentMaster p = em.find(PaymentMaster.class,payment_id);
        em.remove(p);
    
    
    }
    
    @Override
    public String PaymentModeById(Integer payment_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        PaymentMaster p = em.find(PaymentMaster.class,payment_id);
        return p.getPaymentMode();
    
    }


    @Override
    public Collection<PaymentMaster> getAllPaymentMasters() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("PaymentMaster.findAll").getResultList();
    
    
    }

    @Override
    public Collection<PaymentMaster> getPaymentMastersByPaymentMode(String payment_mode) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.

       Collection<PaymentMaster> payment = em.createNamedQuery("PaymentMaster.findByPaymentMode").setParameter("PaymentMode",payment_mode).getResultList();
        return payment;



    }

    @Override
    public PaymentMaster findPaymentMasterById(Integer payment_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(PaymentMaster.class,payment_id);
    }

    @Override
    public void addPaymentDetail(Integer payment_id, Integer cvv, String bank_name, String ifsc_code) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
      PaymentMaster payment = em.find(PaymentMaster.class, payment_id);
      Collection<PaymentDetail> paymentdetails = payment.getPaymentDetailCollection();
      PaymentDetail paymentdetail = new PaymentDetail();
      paymentdetail.setPaymentId(payment);
      paymentdetail.setCvv(cvv);
      paymentdetail.setBankName(bank_name);
      paymentdetail.setIfscCode(ifsc_code);
      paymentdetails.add(paymentdetail);
      payment.setPaymentDetailCollection(paymentdetails);
      em.persist(paymentdetail);
      em.merge(payment);
    
    
    
    }

    @Override
    public void updatePaymentDetail(Integer payment_detail_id, Integer payment_id, Integer cvv, String bank_name, String ifsc_code) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        PaymentDetail pd = em.find(PaymentDetail.class,payment_detail_id);
        PaymentMaster pm = em.find(PaymentMaster.class,payment_id);
        pd.setPaymentId(pm);
        pd.setCvv(cvv);
        pd.setBankName(bank_name);
        pd.setIfscCode(ifsc_code);
        em.merge(pd);
    
    
    
    
    }

    @Override
    public void removePaymentDetail(Integer payment_detail_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
       PaymentDetail paymentdetail = em.find(PaymentDetail.class,payment_detail_id);
       em.remove(paymentdetail);
       
    }

    @Override
    public Collection<PaymentDetail> getAllPaymentDetails() {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       return em.createNamedQuery("PaymentDetail.findAll").getResultList();
    
    
    
    
    }

    @Override
    public Collection<PaymentDetail> getPaymentDetailOfPaymentId(Integer payment_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        PaymentMaster payment = em.find(PaymentMaster.class, payment_id);
        return payment.getPaymentDetailCollection();
    
    
    
    
    
    }

    @Override
    public Collection<PaymentDetail> getPaymentDetailsByCvv(Integer cvv) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
         Collection<PaymentDetail> paymentdetails = em.createNamedQuery("PaymentDetail.findByCvv").setParameter("Cvv",cvv).getResultList();
        return paymentdetails;
   
    }

    @Override
    public Collection<PaymentDetail> getPaymentDetailsByBankName(String bank_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
        Collection<PaymentDetail> paymentdetails = em.createNamedQuery("PaymentDetail.findByBankName").setParameter("BankName",bank_name).getResultList();
        return paymentdetails;
    
    
    
    
    
    }

    @Override
    public Collection<PaymentDetail> getPaymentDetailsByIfscCode(String ifsc_code) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        Collection<PaymentDetail> paymentdetails = em.createNamedQuery("PaymentDetail.findByIfscCode").setParameter("IfscCode",ifsc_code).getResultList();
        return paymentdetails;
    
    
    
    
    }

    @Override
    public PaymentDetail findPaymentDetailById(Integer payment_detail_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    return em.find(PaymentDetail.class,payment_detail_id);
    
    
    
    
    }

    @Override
    public void addRoom(String room_type, Integer category_id,Integer hostel_id, Integer room_price, Boolean room_status, Integer no_of_student) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
      CategoryMaster category = em.find(CategoryMaster.class, category_id);
      Hostel hostel = em.find(Hostel.class, hostel_id);
      Room room = new Room();
      room.setCategoryId(category);
      room.setHostelId(hostel);
      room.setRoomType(room_type);
      room.setRoomPrice(room_price);
       room.setRoomStatus(true);
      room.setNoOfStudent(no_of_student);
      em.persist(room);
     
    
    }

    

    @Override
    public void updateRoom(Integer room_id, String room_type, Integer category_id, Integer hostel_id, Integer room_price, Boolean room_status, Integer no_of_student) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Room room = em.find(Room.class,room_id);
        room.setRoomType(room_type);
        CategoryMaster categoryMaster = em.find(CategoryMaster.class,category_id);
        room.setCategoryId(categoryMaster);
        Hostel hostel = em.find(Hostel.class,hostel_id);
        room.setHostelId(hostel);
        room.setRoomPrice(room_price);
        room.setRoomStatus(true);
        room.setNoOfStudent(no_of_student);
        em.merge(room);
       
    }

    @Override
    public void removeRoom(Integer room_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
       Room room = em.find(Room.class,room_id);
      em.remove(room);
       
    
    }

    
    @Override
    public Collection<Room> getAllRooms() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Room.findAll").getResultList();
    
    
    
    
    }

    @Override
    public Collection<Room> getRoomsOfCategoryId(Integer category_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        CategoryMaster category = em.find(CategoryMaster.class, category_id);
        return category.getRoomCollection();
    
    
    
    
    
    }

    @Override
    public Collection<Room> getRoomsOfHostelId(Integer hostel_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        Hostel hostel = em.find(Hostel.class, hostel_id);
        return hostel.getRoomCollection();
    
    
    
    
    }

    @Override
    public Collection<Room> getRoomsByRoomType(String room_type) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Room> rooms = em.createNamedQuery("Room.findByRoomType").setParameter("RoomType",room_type).getResultList();
        return rooms;
    
    
    
    
    }

    @Override
    public Collection<Room> getRoomsByRoomPrice(Integer room_price) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        Collection<Room> rooms = em.createNamedQuery("Room.findByRoomPrice").setParameter("RoomPrice",room_price).getResultList();
        return rooms;
    
    
    
    }

    @Override
    public Collection<Room> getRoomsByRoomtStatus(Boolean room_status) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Room> rooms = em.createNamedQuery("Room.findByRoomStatus").setParameter("RoomStatus",room_status).getResultList();
        return rooms;
    
    }

    @Override
    public Collection<Room> getRoomsByNoOfStudent(Integer no_of_student) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Room> rooms = em.createNamedQuery("Room.findByNoOfStudent").setParameter("NoOfStudent",no_of_student).getResultList();
        return rooms;
    
    
    
    
    
    }

    @Override
    public Room findRoomById(Integer room_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(Room.class,room_id);
    
    
    
    
    }

    @Override
    public void addStateMaster(String state_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        StateMaster s = new StateMaster();
        s.setStateName(state_name);
        em.persist(s);
    
    
    
    }

    @Override
    public void updateStateMaster(Integer state_id, String state_name) {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
     StateMaster s = em.find(StateMaster.class,state_id);
     s.setStateName(state_name);
     em.merge(s);
    
    
    
    }

    @Override
    public void removeStateMaster(Integer state_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        StateMaster s = em.find(StateMaster.class, state_id);
        em.remove(s);
    
    }
    
    @Override
    public String StateNameById(Integer state_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        StateMaster s = em.find(StateMaster.class, state_id);
        return s.getStateName();
    
    }

    @Override
    public Collection<StateMaster> getAllStateMasters() {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    return em.createNamedQuery("StateMaster.findAll").getResultList();
    
    
    }

    @Override
    public Collection<StateMaster> getStateMastersByStateName(String state_name) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<StateMaster> state = em.createNamedQuery("StateMaster.findByStateName")
                                    .setParameter("StateName", state_name)
                                    .getResultList();
    return state;
    
    
    
    }

    @Override
    public StateMaster findStateMasterById(Integer state_id) {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    return em.find(StateMaster.class, state_id);
    
   
    }

    @Override
    public void addUserMaster(String username,String first_name, String last_name, String address, String phone_no, String dob, String gender, String photo, String user_email, String password, String father_name, String f_occupation, String f_phone_no, String f_email, String mother_name, String m_occupation, String nationality, Integer city_id,Integer state_id, Integer pincode , String admin) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        pb = new Pbkdf2PasswordHashImpl();
        String encryptedpassword = pb.generate(password.toCharArray());
       
      CityMaster city = em.find(CityMaster.class,city_id);
      StateMaster state = em.find(StateMaster.class,state_id);
      Collection<UserMaster> user = city.getUserMasterCollection();
      UserMaster um = new UserMaster();
      
      um.setUsername(username);
      um.setFirstName(first_name);
      um.setLastName(last_name);
      um.setAddress(address);
      um.setPhoneNo(phone_no);
      um.setDob(dob);
      um.setGender(gender);
      um.setPhoto(photo);
      um.setUserEmail(user_email);
      um.setPassword(encryptedpassword);
      um.setFatherName(father_name);
      um.setFOccupation(f_occupation);
      um.setFPhoneNo(f_phone_no);
      um.setFEmail(f_email);
      um.setMotherName(mother_name);
      um.setMOccupation(m_occupation);
      um.setNationality(nationality);
      um.setCityId(city);
      um.setStateId(state);
      um.setPincode(pincode);
      
      em.persist(um);
      user.add(um);
      city.setUserMasterCollection(user);
      em.merge(city);
      
      GroupMaster gm = new GroupMaster();
      gm.setGroupMasterPK(new GroupMasterPK("admin",username));
      gm.setUserMaster(um);
      em.persist(gm);
      System.out.println("Admin Inserted !!!");
      
      
    }

   
    @Override
    public void updateUserMaster(Integer user_id,String first_name, String last_name, String address, String phone_no, String dob, String gender, String photo, String user_email, String password, String father_name, String f_occupation, String f_phone_no, String f_email, String mother_name, String m_occupation, String nationality, Integer city_id, Integer state_id, Integer pincode) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
      
      CityMaster city = em.find(CityMaster.class, city_id);
      StateMaster state = em.find(StateMaster.class,state_id);
       
     UserMaster u = (UserMaster) em.createNamedQuery("UserMaster.findByUserId").setParameter("userId",user_id).getSingleResult();
 
      u.setFirstName(first_name);
      u.setLastName(last_name);
      u.setAddress(address);
      u.setPhoneNo(phone_no);
      u.setDob(dob);
      u.setGender(gender);
      u.setPhoto(photo);
      u.setUserEmail(user_email);
      u.setPassword(password);
      u.setFatherName(father_name);
      u.setFOccupation(f_occupation);
      u.setFPhoneNo(f_phone_no);
      u.setFEmail(f_email);
      u.setMotherName(mother_name);
      u.setMOccupation(m_occupation);
      u.setNationality(nationality);
      u.setCityId(city);
      u.setStateId(state);
      u.setPincode(pincode);
      em.merge(u);
      

    }

    @Override
    public void removeUserMaster(Integer user_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
     UserMaster um = (UserMaster) em.createNamedQuery("UserMaster.findByUserId").setParameter("userId",user_id).getSingleResult();
     em.remove(um);
    
    
    
    }



    @Override
    public Collection<UserMaster> getAllUserMasters() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    return em.createNamedQuery("UserMaster.findAll").getResultList();
    
    
    
    
    }

    @Override
    public Collection<UserMaster> getUserMastersOfCityId(Integer city_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
          CityMaster city = em.find(CityMaster.class, city_id);
     	  return city.getUserMasterCollection();
    
    
    
    
    }

    @Override
    public Collection<UserMaster> getUserMastersOfStateId(Integer state_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
        StateMaster state = em.find(StateMaster.class, state_id);
     	return state.getUserMasterCollection();
    
    
    
    }

    @Override
    public UserMaster getUserMasterByUserName(String username) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        UserMaster usermaster = em.find(UserMaster.class,username);
         return usermaster;
    
    }

    
    @Override
    public Collection<UserMaster> getUserMastersByFirstName(String first_name) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByFirstName")
                                    .setParameter("firstName", first_name)
                                    .getResultList();
    return usermaster;
    
    
    
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByLastName(String last_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByLastname")
                                    .setParameter("lastName", last_name)
                                    .getResultList();
    
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByAddress(String address) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByAddress")
                                    .setParameter("address", address)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByPhoneNo(String phone_no) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByPhoneno")
                                    .setParameter("phoneno", phone_no)
                                    .getResultList();
    
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByDob(String dob) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByDOB")
                                    .setParameter("dob", dob)
                                    .getResultList();
    
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByGender(String gender) {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByGender")
                                    .setParameter("gender", gender)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByPhoto(String photo) {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByPhoto")
                                    .setParameter("Photo", photo)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByUserEmail(String user_email) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByUseremail")
                                    .setParameter("Useremail", user_email)
                                    .getResultList();
    return usermaster;
    
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByPassword(String password) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByPassword")
                                    .setParameter("Password", password)
                                    .getResultList();
    
    return usermaster;
    }

    @Override
    public Collection<UserMaster> getUserMastersByFatherName(String father_name) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByFathername")
                                    .setParameter("father_name",father_name)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByFOccupation(String f_occupation) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByFOccupation")
                                    .setParameter("foccupatione",f_occupation)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByFPhoneNo(String f_phone_no) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByFPhoneno")
                                    .setParameter("FPhoneno ",f_phone_no)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByFEmail(String f_email) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByFemail")
                                    .setParameter("Femail",f_email)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByMotherName(String mother_name) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByMothername")
                                    .setParameter("Mothername",mother_name)
                                    .getResultList();
    
    return usermaster;
    }

    @Override
    public Collection<UserMaster> getUserMastersByMOccupation(String m_occupation) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByMOccupation")
                                    .setParameter("MOccupation",m_occupation)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByNationality(String nationality) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByNationality")
                                    .setParameter("Nationality",nationality)
                                    .getResultList();
    
    return usermaster;
    
    }

    @Override
    public Collection<UserMaster> getUserMastersByPincode(Integer pincode) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    
    Collection<UserMaster> usermaster = em.createNamedQuery("UserMaster.findByPincode")
                                    .setParameter("Pincode",pincode)
                                    .getResultList();
    return usermaster;
    
    }

    @Override
    public UserMaster findUserMasterById(Integer user_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
    
    return em.find(UserMaster.class, user_id);
    
    
    }

    @Override
    public void addEvents(String event_name, String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Events e = new Events();
        e.setEventName(event_name);
        e.setDescription(description);
        em.persist(e);
    
    
    }

    @Override
    public void updateEvents(Integer event_id, String event_name, String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Events e = em.find(Events.class,event_id);
        e.setEventName(event_name);
        e.setDescription(description);
        em.merge(e);
    
    }

    @Override
    public void removeEvents(Integer event_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Events e = em.find(Events.class,event_id);
        em.remove(e);
    
    }

    @Override
    public String EventNameById(Integer event_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Events e = em.find(Events.class,event_id);
        return e.getEventName();
    
    }

    @Override
    public String DesriptionById(Integer event_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        
        Events e = em.find(Events.class,event_id);
        return e.getDescription();
        
    }

    
    
    @Override
    public Collection<Events> getAllEvents() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Events.findAll").getResultList();
    }

    @Override
    public Collection<Events> getEventsByEventName(String event_name) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Collection<Events> event = em.createNamedQuery("Events.findByEventName").setParameter("EventName",event_name).getResultList();
        return event;
    
    }

    @Override
    public Collection<Events> getEventsByDescription(String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        Collection<Events> event = em.createNamedQuery("Events.findByDescription").setParameter("Description",description).getResultList();
        return event;
    
    }

    @Override
    public Events findEventsById(Integer event_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        return em.find(Events.class, event_id);
    
    }

    @Override
    public void addNotice(String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Notice n = new Notice();
        n.setDescription(description);
        em.persist(n);
    
    }

    @Override
    public void updateNotice(Integer notice_id, String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Notice n = em.find(Notice.class,notice_id);
        n.setDescription(description);
        em.merge(n);
    
    
    }

    @Override
    public void removeNotice(Integer notice_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
         Notice n = em.find(Notice.class,notice_id);
        em.remove(n);
        
    }
    
    @Override
    public String NDesriptionsById(Integer notice_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        
        Notice n = em.find(Notice.class,notice_id);
        return n.getDescription();
        
    }

    @Override
    public Collection<Notice> getAllNotice() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Notice.findAll").getResultList();
    
    }

    @Override
    public Collection<Notice> getNoticeByDescription(String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
         Collection<Notice> notice = em.createNamedQuery("Notice.findByDescription").setParameter("Description",description).getResultList();
        return notice;
    
    }

    @Override
    public Notice findNoticeById(Integer notice_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
       return em.find(Notice.class,notice_id);
    
    }

    @Override
    public void addRules(String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Rules r = new Rules(); 
        r.setDescription(description);
        em.persist(r);
    
    }

    @Override
    public void updateRules(Integer rules_id, String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Rules r = em.find(Rules.class,rules_id);
        r.setDescription(description);
        em.merge(r);
    
    
    }

    @Override
    public void removeRules(Integer rules_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Rules r = em.find(Rules.class,rules_id);
        em.remove(r);
    
    
    }
    
    @Override
    public String DesriptionsById(Integer rules_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        
        Rules r = em.find(Rules.class,rules_id);
        return r.getDescription();
        
    }


    @Override
    public Collection<Rules> getAllRules() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Rules.findAll").getResultList();
    
    }

    @Override
    public Collection<Rules> getRulesByDescription(String description) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Collection<Rules> rule = em.createNamedQuery("Rules.findByDescription").setParameter("Description",description).getResultList();
        return rule;
    
    
    }

    @Override
    public Rules findRulesById(Integer rules_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.find(Rules.class, rules_id);
    
    }

    @Override
    public void addTimelist(String time, String day) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Timelist tl = new Timelist();
        tl.setTime(time);
        tl.setDay(day);
        em.persist(tl);
    
    }

    @Override
    public void updateTimelist(Integer timelist_id, String time, String day) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        Timelist tl = em.find(Timelist.class,timelist_id);
        tl.setTime(time);
        tl.setDay(day);
        em.merge(tl);
    
    
    }

    @Override
    public void removeTimelist(Integer timelist_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Timelist tl = em.find(Timelist.class,timelist_id);
        em.remove(tl);
    
    }

    @Override
    public String TimeById(Integer timelist_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Timelist tl = em.find(Timelist.class,timelist_id);
        return tl.getTime();
    
    }

    @Override
    public String DayById(Integer timelist_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       Timelist tl = em.find(Timelist.class,timelist_id);
        return tl.getDay();
    
    }
    
    

    @Override
    public Collection<Timelist> getAllTimelist() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Timelist.findAll").getResultList();
    
    }

    @Override
    public Collection<Timelist> getTimelistByTime(String time) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Collection<Timelist> timelist = em.createNamedQuery("Timelist.findByTime").setParameter("Time",time).getResultList();
        return timelist;
    
    }

    @Override
    public Collection<Timelist> getTimelistByDay(String day) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        Collection<Timelist> timelist = em.createNamedQuery("Timelist.findByDay").setParameter("Day",day).getResultList();
        return timelist;
       
    }

    @Override
    public Timelist findTimelistById(Integer timelist_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
         return em.find(Timelist.class, timelist_id);
    
    }

   

    @Override
    public void removeFeedback(Integer feedback_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.

         Feedback f = em.find(Feedback.class,feedback_id);
        em.remove(f);

    }

    @Override
    public Collection<Feedback> getAllFeedback() {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
         return em.createNamedQuery("Feedback.findAll").getResultList();
    
    }

    @Override
    public Feedback findFeedbackById(Integer feedback_id) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
   
        return em.find(Feedback.class,feedback_id);
    
    }

    @Override
    public void addFacilities(Integer hostel_id, String facilities_detail) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
      Hostel hostel = em.find(Hostel.class,hostel_id);
      Collection<Facilities> facilities = hostel.getFacilitiesCollection();
      Facilities f = new Facilities();
      f.setHostelId(hostel);
      f.setFacilitiesDetail(facilities_detail);
      facilities.add(f);
      hostel.setFacilitiesCollection(facilities);
      em.persist(f);
      em.merge(hostel);
    
    
    }

    @Override
    public void updateFacilities(Integer facilities_id, Integer hostel_id, String facilities_detail) {
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       
        Facilities facilities = em.find(Facilities.class,facilities_id);
        Hostel hostel = em.find(Hostel.class,hostel_id);
        facilities.setHostelId(hostel);
        facilities.setFacilitiesDetail(facilities_detail);
        em.merge(facilities);
        
    
    
    
    }

    @Override
    public void removeFacilities(Integer facilities_id) {
      //  throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.

      Facilities facilities = em.find(Facilities.class,facilities_id);
      em.remove(facilities);


    }

    @Override
    public Collection<Facilities> getAllFacilities() {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
        return em.createNamedQuery("Facilities.findAll").getResultList();
    
    
    }

    @Override
    public Collection<Facilities> getFacilitiesOfHostelId(Integer hostel_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        
        Hostel hostel = em.find(Hostel.class,hostel_id);
        return hostel.getFacilitiesCollection();
    
    }

    @Override
    public Collection<Facilities> getFacilitiesByFacilitiesDetail(String facilities_detail) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       Collection<Facilities> facilities = em.createNamedQuery("Facilities.findByFacilitiesDetail").setParameter("FacilitiesDetail",facilities_detail).getResultList();
        return facilities;
    
    }

    @Override
    public Facilities findFacilitiesById(Integer facilities_id) {
       // throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    
       return em.find(Facilities.class,facilities_id);
    
    }

    
    
    
}
